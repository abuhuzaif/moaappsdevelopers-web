...
// Photos already have the city name + "Saudi Arabia" label baked in
// (designed cards), so no HTML text overlay is rendered on top of them.
const CITY_CARDS = [
  { name: "Madina", image: "madina" },
  { name: "Riyadh", image: "riyadh" },
  { name: "Jeddah", image: "jeddah" },
  { name: "Dammam", image: "dammam" },
  { name: "Khobar", image: "khobar" },
  { name: "Jubail", image: "jubail" },
  { name: "Yanbu", image: "yanbu" },
];
        ...
        <div className="hero-photo-box">
          ...
          >
            <nav className="hero-nav-row">
              <a href="/ksa-connect" className="hero-brand-logo">
                <span className="hero-brand-icon">🌴</span>
                KSA<span className="gold">-Connect</span>
              </a>
              <div className="hero-nav-links">
                <a href="#listings">
                  <span>⊞</span> Browse Ads
                </a>
                <a href="/ksa-connect/post">
                  <span>+</span> Post an Ad
                </a>
                {user ? (
                  <button onClick={() => signOutUser()}>
                    <span>👤</span> {user.displayName?.split(" ")[0] ?? "Account"}
                  </button>
                ) : (
            ...
            <div className="container" style={{ paddingBottom: 8, paddingTop: 68, margin: "0 auto 0 0" }}>
              <h1 style={{ textAlign: "left", maxWidth: 420 }}>
                Your Connection <span className="gold">Across Saudi Arabia</span>
              </h1>
              <p style={{ textAlign: "left", maxWidth: 340, margin: "0 0 20px" }}>
              ...
              <div className="hero-stats-badges">
                <div className="hero-stat-badge">
                  <span className="hero-stat-icon hero-stat-icon-purple">🏢</span>
                  <div>
                ...
                <div className="hero-stat-badge">
                  <span className="hero-stat-icon hero-stat-icon-green">📈</span>
                  <div>
                ...
                <div className="hero-stat-badge">
                  <span className="hero-stat-icon hero-stat-icon-gold">⊞</span>
                  <div>
          ...
          {/* ── Cities Covered — clickable, filters listings by city.
              Placeholder colored cards for now; swap in real photos once
              provided (see CITY_CARDS above). ── */}
          <div className="hero-cities-row">
            <div className="hero-cities-label">
              <span className="hero-cities-icon">📍</span>
              Cities
              <br />
              Covered
            </div>
            {CITY_CARDS.map((city) => (
              <button
                ...
                style={{ backgroundImage: `url('/images/cities/${city.image}.jpg')` }}
                onClick={() => handleCityClick(city.name)}
                aria-label={`Browse listings in ${city.name}`}
              />
            ))}
          </div>

          {/* ── Search bar + trending keywords — sits below the photo now,
              so it never covers the mug/pin/phone artwork ── */}
